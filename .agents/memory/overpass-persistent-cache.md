...
**Test-isolation gotcha (route.test.ts):** the in-memory L1 map survives across tests in the same vitest process, and `clearFixtures` only deletes the Postgres `overpass_cache` rows (by key), NOT the L1 entries. So two tests that mock different Overpass payloads under the SAME padded bbox key will shadow each other — whichever runs first warms L1, and the second silently reads the stale payload. **How to apply:** give each Overpass-backed test its own distinct request coordinates (a different bbox → different cache key), rather than reusing another test's node coords. This is why the live-fallback test uses lat 41 while the unsnappable test uses lat 40.
