...
Automatic backup (July 2026): `scripts/post-merge.sh` runs `./scripts/push-to-github.sh` after every task merge, so origin/main stays current without manual pushes; a stale token fails post-merge setup loudly (by design) — renew GITHUB_PUSH_TOKEN, then re-run the push manually.
