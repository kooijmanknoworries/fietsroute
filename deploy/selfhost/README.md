...
```
                 http://LAN-IP  (or https://your-domain later)
                          │
                    ┌─────▼─────┐
...
cp .env.example .env
nano .env        # set POSTGRES_PASSWORD, CLERK_* keys, PUBLIC_DOMAIN
...
## Going HTTPS with your custom domain

Prerequisites you do once (outside this stack):
1. **DNS**: create an `A` record for your domain (e.g. `fiets.example.com`)
   pointing at your static public IP.
2. **Router**: forward external TCP ports **80 and 443** to this machine.
...
```bash
nano .env      # set DOMAIN=fiets.example.com and PUBLIC_DOMAIN=fiets.example.com
docker compose build mobile        # rebake the domain into the bundles
...
persist in the `caddy_data` volume). Port 80 redirects to 443.

### Clerk: development vs production instance

| | Development (`pk_test_...`) | Production (`pk_live_...`) |
|---|---|---|
| Works on | any address (LAN IP, any domain) | only your custom domain |
| `CLERK_PROXY_URL` | leave **empty** | set to `/api/__clerk` |
| Limits | Clerk dev-instance limits (user cap, "development" banner) | none |

To switch to production once the domain is live:
1. In the Clerk dashboard, create/activate the production instance for your
   domain and copy its `pk_live_...` / `sk_live_...` keys.
2. In `.env`: set both `CLERK_*` keys and `CLERK_PROXY_URL=/api/__clerk`.
3. Rebuild the images that bake the key in, and restart:
   ```bash
   docker compose build web mobile && docker compose up -d
   ```
